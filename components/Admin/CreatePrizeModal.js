/* This example requires Tailwind CSS v2.0+ */
import { Fragment, useRef, useState } from "react";
import { Dialog, Transition } from "@headlessui/react";
import { useSession } from "next-auth/client"
import axios from "axios";

import { TournamentPrizeType } from "../../types";
import LuxInputBox from "../LuxInputBox";

export default function CreatePrizeModal({
  tournament,
  openModal,
  setOpenModal,
  innerRef,
}) {
  const [session] = useSession();
  const [prizeId, setPrizeId] = useState(0);
  const [prizeTypeId, setPrizeTypeId] = useState(0);
  const [productId, setProductId] = useState(0);
  const [prizeValue, setPrizeValue] = useState(0);
  const [prizeLevel, setPrizeLevel] = useState(0);
  const [prizeTitle, setPrizeTitle] = useState("");
  const [prizeDesc, setPrizeDesc] = useState("");
  const token = session ? session.accessToken : null;

  const prizeAddOrUpdate = async (e) => {
    e.preventDefault();

    const data = JSON.stringify({
      idTournament: tournament[0].id,
      idTournamentPrize: prizeId,
      idTournamentPrizeType: prizeTypeId,
      idProduct: productId,
      tpValue: prizeValue,
      tpLevel: prizeLevel,
      tpTitle: prizeTitle,
      tpDescription: prizeDesc,
    });

    const res = await axios
      .post(`${process.env.BASE_API_URL}/Tournament/PrizeAddOrUpdate`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      })
      .catch((err) => {
      });
    if (res) {
      router.push("/admin/tournament");
    }
  };

  return (
    <Transition.Root show={openModal} as={Fragment}>
      <Dialog
        as="div"
        className="fixed z-10 inset-0 overflow-y-auto"
        initialFocus={innerRef}
        onClose={setOpenModal}
      >
        <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
            enterTo="opacity-100"
            leave="ease-in duration-200"
            leaveFrom="opacity-100"
            leaveTo="opacity-0"
          >
            <Dialog.Overlay className="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" />
          </Transition.Child>

          {/* This element is to trick the browser into centering the modal contents. */}
          <span
            className="hidden sm:inline-block sm:align-middle sm:h-screen"
            aria-hidden="true"
          >
            &#8203;
          </span>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
            enterTo="opacity-100 translate-y-0 sm:scale-100"
            leave="ease-in duration-200"
            leaveFrom="opacity-100 translate-y-0 sm:scale-100"
            leaveTo="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
          >
            <div className="inline-block align-bottom bg-primary rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-primary px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 w-full">
                    <Dialog.Title
                      as="h3"
                      className="text-lg leading-6 font-medium text-white"
                    >
                      Create Prize
                    </Dialog.Title>
                    <div className="flex flex-col mt-2 space-y-2">
                      <label
                        htmlFor="trnType"
                        className="text-white text-left text-sm font-medium"
                      >
                        Prize Type
                      </label>
                      <select
                        name="trnType"
                        id="trnType"
                        className="mt-1 p-2 block w-full border-gray-400 border-[1px] rounded-md shadow-sm 
                   focus:ring-opacity-50 bg-primary text-gray-300"
                        value={prizeTypeId}
                        onChange={(e) => setPrizeTypeId(e.target.value)}
                        required
                      >
                        {TournamentPrizeType.map((option, index) => (
                          <option
                            key={index}
                            value={option.i}
                            onChange={() => setPrizeTypeId(e.target.value)}
                          >
                            {option.v}
                          </option>
                        ))}
                      </select>
                      <label className="flex flex-col space-y-2 text-white text-left text-sm">
                        <p>Prize Title</p>
                        <LuxInputBox type="text" value={prizeTitle} onChange={(e) => setPrizeTitle(e.target.value)}/>
                      </label>
                      <label className="flex flex-col space-y-2 text-white text-left text-sm">
                        <p>Prize Value</p>
                        <LuxInputBox type="number" value={prizeValue} onChange={(e) => setPrizeValue(e.target.value)}/>
                      </label>
                      <label className="flex flex-col space-y-2 text-white text-left text-sm">
                        <p>Prize Level</p>
                        <LuxInputBox type="number" value={prizeLevel} onChange={(e) => setPrizeLevel(e.target.value)}/>
                      </label>
                      <label className="flex flex-col space-y-2 text-white text-left text-sm">
                        <p>Prize Description</p>
                        <textarea
                          name="prizeDesc"
                          id=""
                          cols="30"
                          rows="5"
                          className="bg-transparent border-gray-300 border rounded-lg"
                          value={prizeDesc}
                          onChange={(e) => setPrizeDesc(e.target.value)}
                        ></textarea>
                      </label>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  className="mt-3 w-full inline-flex justify-center rounded-md border 
                  border-gray-300 shadow-sm px-4 py-2 bg-rpimary text-base font-medium
                   text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 
                   focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm"
                   onClick={prizeAddOrUpdate}
                >
                  Create
                </button>
              </div>
            </div>
          </Transition.Child>
        </div>
      </Dialog>
    </Transition.Root>
  );
}
